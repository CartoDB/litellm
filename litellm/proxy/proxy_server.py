...
try:
    ...
    if is_non_root:
        # CARTO: Allow configurable UI path via env var (default: /var/lib/litellm/ui)
        # This enables using /tmp/litellm_ui in CARTO's Dockerfile.non_root
        runtime_ui_path = os.getenv("LITELLM_UI_PATH", "/var/lib/litellm/ui")
    ...
    # In non-root Docker, we restructure in the configured runtime UI path.
    try:
        # CARTO: Use env var for path comparison (matches runtime_ui_path logic above)
        configured_ui_path = os.getenv("LITELLM_UI_PATH", "/var/lib/litellm/ui")
        if is_non_root and ui_path == configured_ui_path:
            verbose_proxy_logger.info(
...
def get_image():
    ...
    is_non_root = os.getenv("LITELLM_NON_ROOT", "").lower() == "true"
    # CARTO: Allow configurable assets path via env var (default: /var/lib/litellm/assets)
    assets_dir = os.getenv("LITELLM_ASSETS_PATH", "/var/lib/litellm/assets") if is_non_root else current_dir
